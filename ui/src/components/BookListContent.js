import React from 'react';
import {
    Cells,
    CellsTitle,
    Cell,
    CellHeader,
    CellBody,
    CellFooter,
} from 'react-weui';
import iconSrc from '../images/icon_black/fuwudiqiu.png';

import {
    Panel,
    PanelHeader,
    PanelBody,
    PanelFooter,
    MediaBox,
    MediaBoxHeader,
    MediaBoxBody,
    MediaBoxTitle,
    MediaBoxDescription,
    MediaBoxInfo,
    MediaBoxInfoMeta,
} from 'react-weui';


import Title from '../components/Title'

// const appMsgIcon = <img src="data:image/png;base64," />

import msgIcon from '../images/Weather Forecast.png';
const appMsgIcon = <img src={msgIcon} alt=""/>



const CellMore = () => (
    <Cell access link>
        <CellBody>更多</CellBody>
        <CellFooter />
    </Cell>
)

class BookListContent extends React.Component {
    render() {
        return (
            <div>
                <Title></Title>
                <Panel>
                    <PanelBody>
                        <MediaBox type="appmsg" href="javascript:void(0);">
                            <MediaBoxHeader>{appMsgIcon}</MediaBoxHeader>
                            <MediaBoxBody>
                                <MediaBoxTitle>小鹿与小熊</MediaBoxTitle>
                                <MediaBoxDescription>
                                     小鹿与小熊的故事出自安徒生童话108卷
                                </MediaBoxDescription>
                            </MediaBoxBody>
                        </MediaBox>
                        <MediaBox type="appmsg" href="javascript:void(0);">
                            <MediaBoxHeader>{appMsgIcon}</MediaBoxHeader>
                            <MediaBoxBody>
                                <MediaBoxTitle>小黑</MediaBoxTitle>
                                <MediaBoxDescription>
                                     小黑的故事出自安徒生童话109卷
                                </MediaBoxDescription>
                            </MediaBoxBody>
                        </MediaBox>
                        <MediaBox type="appmsg" href="javascript:void(0);">
                            <MediaBoxHeader>{appMsgIcon}</MediaBoxHeader>
                            <MediaBoxBody>
                                <MediaBoxTitle>小王子</MediaBoxTitle>
                                <MediaBoxDescription>
                                     小王子是经典成人童话故事
                                </MediaBoxDescription>
                            </MediaBoxBody>
                        </MediaBox>
                        <MediaBox type="appmsg" href="javascript:void(0);">
                            <MediaBoxHeader>{appMsgIcon}</MediaBoxHeader>
                            <MediaBoxBody>
                                <MediaBoxTitle>小狼与猪</MediaBoxTitle>
                                <MediaBoxDescription>
                                     改编于三只小猪
                                </MediaBoxDescription>
                            </MediaBoxBody>
                        </MediaBox>
                    </PanelBody>
                </Panel>
            </div>
        );
    }
}


export default BookListContent;